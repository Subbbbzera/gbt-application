...
const Favorites = () => {
  ...
  const { search, selectedCategories, saleTypeFilter, rentPriceRange, buyPriceRange, sortBy, currentPage } = filters;
  const cardsPerPage = 12;

  
  const allRentPrices = favorites.map(c => parseFloat(c.price_per_day)).filter(p => !isNaN(p) && p > 0);
  const absoluteMaxRent = allRentPrices.length ? Math.max(...allRentPrices) : 10000;

  const allBuyPrices = favorites.map(c => parseFloat(c.price_buy)).filter(p => !isNaN(p) && p > 0);
  ...
  let filteredCards = favorites.filter(card => {
    ...
    const matchSaleType = saleTypeFilter === "all" || card.sale_type === "both" || card.sale_type === saleTypeFilter;

    const rentPrice = parseFloat(card.price_per_day) || 0;
    const matchRentPrice = rentPrice >= (parseFloat(rentPriceRange.min) || 0) && rentPrice <= (parseFloat(rentPriceRange.max) || Infinity);

    const buyPrice = parseFloat(card.price_buy) || 0;
    const matchBuyPrice = buyPrice >= (parseFloat(buyPriceRange.min) || 0) && buyPrice <= (parseFloat(buyPriceRange.max) || Infinity);
  ...
  const FilterFields = () => (
    <>
      ...
      <div className="border-b pb-4">
        <button onClick={() => toggleSection('saleType')} className="w-full flex justify-between items-center py-2 group">
      ...
      <div className="border-b pb-4">
        <button onClick={() => toggleSection('categories')} className="w-full flex justify-between items-center py-2 group">
      ...
      <button onClick={() => setIsMobileFilterOpen(false)} className="lg:hidden w-full py-3 bg-emerald-600 text-white rounded-lg font-bold mt-4">
        Застосувати
      </button>
    </>
  );
  ...
  return (
    <div className="flex flex-col gap-8 max-w-[1440px] mx-auto px-4 md:px-8 py-6 lg:py-10 pt-24 min-h-screen bg-gray-100">


      {}
      <div className="flex lg:hidden justify-between items-center bg-white p-2 border rounded-xl shadow-sm">
        <p className="text-gray-600 text-sm">Улюблених: <b>{filteredCards.length}</b></p>
        <button 
      ...
      <div className="flex gap-8">
        {}
        <div className="hidden lg:block w-1/4 h-fit bg-white border border-emerald-100 rounded-2xl p-6 flex-shrink-0 space-y-6 sticky top-28 shadow-sm">
          <FilterFields />
        </div>

        {}
        <div className={`fixed inset-0 z-[100] lg:hidden transition-opacity duration-300 ${isMobileFilterOpen ? "opacity-100 visible" : "opacity-0 pointer-events-none invisible"}`}>
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setIsMobileFilterOpen(false)} />
          <div className={`absolute top-0 left-0 bottom-0 w-[80%] max-w-[320px] bg-white shadow-2xl p-6 overflow-y-auto space-y-6 transition-transform duration-300 ease-in-out ${isMobileFilterOpen ? "translate-x-0" : "-translate-x-full"}`}>
            <FilterFields />
          </div>
        </div>
        ...
        <div className="w-full lg:w-3/4">
          ...
          {favorites.length === 0 ? (
            <div className="flex flex-col items-center justify-center mt-20">
              <p className="text-xl text-gray-500">У списку поки що нічого немає</p>
            </div>
          ) : currentCards.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-xl text-gray-600">Нічого не знайдено за фільтрами</p>
            </div>
          ) : (
          ...
          {totalPages > 1 && (
            <div className="flex justify-center gap-3 mt-10">
              <button
                ...
                className="px-4 py-2 rounded-lg border bg-white text-emerald-700 border-emerald-700 hover:bg-emerald-100 disabled:opacity-50 transition"
              >
                Назад
              </button>
              {Array.from({ length: totalPages }, (_, i) => (
                <button
                  key={i + 1}
                  onClick={() => updateFilters({ currentPage: i + 1})}
                  className={`px-4 py-2 rounded-lg border ${currentPage === i + 1 ? "bg-emerald-700 text-white" : "bg-white text-emerald-700 border-emerald-700 hover:bg-emerald-100"} transition`}
                >
                  {i + 1}
                </button>
              ))}
